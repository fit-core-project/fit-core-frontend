...
export interface RoutineRepository {
    generateDraft(payload: unknown): Promise<RoutineDraft>
}

// 2. Fixture (가짜 데이터) 구현체
export class FixtureRoutineRepository implements RoutineRepository {
    async generateDraft(payload: unknown): Promise<RoutineDraft> {
        ...
        const p = payload as { target_muscles?: string[] }
        const simulateError = p.target_muscles?.includes("error_trigger")
